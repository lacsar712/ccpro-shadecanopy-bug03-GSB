...
def dashboard_stats(request):
    ...
    todays_cycles = IrrigationCycle.objects.filter(
    ...
    data = {
        ...
        "irrigationScheduledToday": todays_cycles.filter(
            status=IrrigationCycle.STATUS_SCHEDULED
        ).count(),
        # 与轮灌列表同一套读出口径：逐行经 as_water 读出后求和
        "irrigationTodayLiters": sum(
            as_water(v) for v in todays_cycles.values_list("water_liters", flat=True)
        ),
    }
